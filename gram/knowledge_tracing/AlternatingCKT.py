    ...
    def compute_lm_loss(
        ...
        if weight_count == 'linear':
            ...
            log_add_weight = torch.tensor([np.log(self.item_count[int(i.cpu().numpy())] + 1)
                                           / (max(np.log(np.asarray(list(self.item_count.values())))) + 1) for i in x]).type(torch.FloatTensor)
            ...
            sqrt_weight = torch.tensor([np.sqrt(self.item_count[int(i.cpu().numpy())])
                                        / max(np.sqrt(np.asarray(list(self.item_count.values())))) for i in x]).type(torch.FloatTensor)
            sqrt_weighted_loss = torch.dot(sqrt_weight.cuda(), loss_before_reduction)
            return sqrt_weighted_loss
        else:
            loss = F.mse_loss(logit, label, reduction="sum")
            return loss
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, optimizer_idx: int=0) -> torch.Tensor:
        ...
        if len(self.train_dataloader.dataloader) - 1 == batch_idx:
            ...
            train_loader = DataLoader(
                TensorDataset(torch.tensor(np.asarray(self.non_cold_start_ids), dtype=torch.float).cuda(),
