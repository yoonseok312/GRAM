...
from repoc_content_kt.scripts.helpers.mlp_MNIST import LightningMLPD, LightningMLP_StepAlternate
...
def main():
    ...
    if mode == 'alt':
        ...
        exp_name = f"MNIST_adam_fix_D_{config.optimizer}_lr{config.ktlr}_m{config.momentum}"
    else:
        AssertionError()
    data_size = 50
    batch_size = 64
    ...
    model_alternate = LightningMLP_StepAlternate(config, train_dict,test_dict,train_length,test_length)
    ...
    if mode == 'alt':
        trainer_alternate = pl.Trainer(
            ...
            max_epochs=epochs,
            gpus=None if config.gpus is None else str(config.gpus),
